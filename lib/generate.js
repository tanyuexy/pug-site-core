...
export async function buildFn() {
  ...
  let languageData = (await import(paths.languageData)).default;
  let languageList = isLinux() ? await getLanguageListFromApi() : config.languageList;
  await async.each(languageList, async (lang) => {
...
export async function buildStatic() {
  ...
  try {
    ...
    let languageList = isLinux() ? await getLanguageListFromApi() : config.languageList;
    // 处理每种语言
    await async.eachSeries(languageList, async (lang) => {
      try {
